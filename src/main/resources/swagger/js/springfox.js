window.onload = () => {
  const buildSystemAsync = async (baseUrl) => {
    try {
      //const configUIResponse = await fetch(baseUrl + "/webjars/swagger-resources/configuration/ui");
      const configUIResponse = await fetch(baseUrl + "/swagger-resources/configuration/ui");
      const configUI = await configUIResponse.json();

      //const configSecurityResponse = await fetch(baseUrl + "/webjars/swagger-resources/configuration/security");
      ...
      //const resourcesResponse = await fetch(baseUrl + "/webjars/swagger-resources");
...
/*      const retryURL = await prompt(
        ...
        let url = window.location.origin + ctxPath + "/webjars";
        console.log("springfox.js.url: " + url);

      //return buildSystemAsync(url);
    }
  };
  ...
  const getBaseURL = () => {
    ...
    const ctxPath = location.href.substring(offset,location.href.indexOf('/',offset+1));
    const url = window.location.origin + ctxPath;
    console.log('springfox.js / url : ' + url);
